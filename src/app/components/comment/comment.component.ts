...
export class CommentComponent implements OnInit {
	...
	addComment(){
		if(this.add)
			this.commentService.save(this.userDataProvider.getToken(), this.story, this.addCommentContent).then(res => this.handleAddComment(res));
		else
			this.commentService.update(this.userDataProvider.getToken(), this.story, this.addCommentContent).then(res => this.handleAddComment(res));
	}
	...
	private handleComments(res:ProxyResponse<CommentContent>){
		if(res) {
			if(this.comments.length == 0) {
				...
				for(var i=this.comments.length % 10; i<res.getList().length; i++){
