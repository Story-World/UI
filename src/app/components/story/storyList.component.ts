...
export class StoryListComponent implements OnInit {
  ...
  constructor(private userDataProvider: UserDataProvider, private storyService: StoryService) {
    this.size = 10;
    this.filters = false;
    this.storyService.getStories(this.page, this.size, null).then(res => this.handleGetStories(res));
  }
  ...
  pageChanged(event: String) {
    this.storyService.getStories(this.page, this.size, this.text).then(res => this.handleGetStories(res));
  }
